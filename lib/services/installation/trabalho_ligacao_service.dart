...
// ═══════════════════════════════════════════════════════
// TRABALHO LIGAÇÃO SERVICE - CRUD
// ═══════════════════════════════════════════════════════

class TrabalhoLigacaoService {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  final String _collection = 'trabalhos_ligacao';

  // ────── CREATE ──────

  /// Criar novo trabalho de ligação
  Future<String> createTrabalho(TrabalhoLigacao trabalho) async {
    try {
  ...
  Future<void> createTrabalhosBatch(List<TrabalhoLigacao> trabalhos) async {
    try {
  ...
  Future<TrabalhoLigacao?> getTrabalhoById(String trabalhoId) async {
    try {
      final doc =
      ...
      return TrabalhoLigacao.fromFirestore(doc);
    } catch (e) {
  ...
  Future<List<TrabalhoLigacao>> getTrabalhosByTurbina(String turbinaId) async {
    try {
      ...
      return snapshot.docs
          .map((doc) => TrabalhoLigacao.fromFirestore(doc))
          .toList();
    } catch (e) {
      throw Exception('Erro ao obter trabalhos da turbina: $e');
    }
  }

  /// Obter trabalho específico por ligação
  Future<TrabalhoLigacao?> getTrabalhoByLigacao(
    String turbinaId,
    String componenteA,
    String componenteB,
  ) async {
    try {
      ...
      return TrabalhoLigacao.fromFirestore(snapshot.docs.first);
    } catch (e) {
  ...
  Stream<List<TrabalhoLigacao>> streamTrabalhosByTurbina(String turbinaId) {
    return _firestore
        .collection(_collection)
        .where('turbinaId', isEqualTo: turbinaId)
        .orderBy('createdAt', descending: false)
  ...
  Future<void> updateTrabalho(
      String trabalhoId, TrabalhoLigacao trabalho) async {
    try {
  ...
  Future<void> deleteTrabalhosByTurbina(String turbinaId) async {
    try {
      ...
      throw Exception('Erro ao deletar trabalhos da turbina: $e');
    }
  }
  ...
  Future<List<TrabalhoLigacao>> getTrabalhosIncompletos(
  ...
  Future<List<TrabalhoLigacao>> getTrabalhosPendentes(String turbinaId) async {
    try {
      final snapshot = await _firestore
          .collection(_collection)
          .where('turbinaId', isEqualTo: turbinaId)
      ...
      final trabalhos = snapshot.docs
          .map((doc) => TrabalhoLigacao.fromFirestore(doc))
          .where((trabalho) => trabalho.tipo == null)
          .toList();
